...
# Добавлена новая функция для проверки добавления питомца с невалидным ключом авторизации


    def create_pet_simple_with_invalid_authkey(self, auth_key: json, name: str, animal_type: str,
                    age: int) -> json:
        """Метод отправляет (постит) на сервер данные о добавляемом питомце (без ключа авторизации) ,без фотографии и возвращает статус
        запроса на сервер и результат в формате JSON с данными добавленного питомца"""
